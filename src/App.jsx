...
function App() {
  return (
    <div className="bg-white text-slate-900 overflow-x-hidden selection:bg-blue-600 selection:text-white font-sans">
      ...
      <section className="relative min-h-[100svh] flex items-center px-5 bg-slate-950 pt-20">
        ...
        <div className="max-w-7xl mx-auto w-full relative z-10 text-center md:text-left">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <div className="inline-block px-3 py-1 bg-blue-600 text-white rounded text-[10px] font-black mb-6 uppercase tracking-[0.2em]">Engineering & Parenting Tech</div>
            <h1 className="text-[clamp(2.1rem,8vw,8rem)] font-black text-white leading-[1.1] md:leading-[1] tracking-tighter mb-8 italic uppercase break-keep">
            ...
            <button onClick={openChat} className="w-full md:w-auto flex items-center justify-center gap-3 text-lg font-black bg-white text-slate-900 px-10 py-5 rounded-2xl shadow-2xl transition-transform active:scale-95">
              프로젝트 컨설팅 시작 <FiArrowRight />
            </button>
          </motion.div>
        </div>
      </section>

      {/* 2. Solutions: 베베헬퍼 상세 반영 */}
      <section id="services" className="py-24 md:py-32 px-5 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="mb-16 md:mb-20 text-center md:text-left">
            ...
            <p className="mt-4 text-slate-400 font-bold uppercase text-[10px] tracking-widest">SI 실무와 자체 서비스 R&D를 병행하는 기술 그룹</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="p-10 rounded-[40px] bg-slate-900 text-white flex flex-col justify-between min-h-[380px]">
              ...
              <div>
                ...
                <p className="text-slate-400 text-sm leading-relaxed break-keep">광고용 랜딩페이지, 소규모 이벤트 페이지 등 자잘한 업무도 환영합니다. 정예 인력이 합리적 비용으로 빠르게 해결합니다.</p>
              </div>
            </div>
            <div className="p-10 rounded-[40px] bg-slate-50 text-slate-900 border border-slate-100 flex flex-col justify-between min-h-[380px]">
              ...
              <div>
                <h3 className="text-2xl font-black mb-4 italic">Enterprise Design</h3>
                <p className="text-slate-500 text-sm leading-relaxed break-keep">향후 금융권 대규모 SI 프로젝트 참여 예정인 전문 인력이 비즈니스 규모에 맞는 확장 가능한 시스템을 설계합니다.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* 3. Tech Stack: 가로 뭉침 방지 (Grid 1열 강제) */}
      ...
      <section id="faq" className="py-24 md:py-32 px-5 bg-white">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-12 text-left">
            {[
              { q: "정말 자잘한 일도 정성껏 해주시나요?", a: "네, 랜딩페이지 한 장이나 간단한 폼 구축 같은 가벼운 업무도 환영합니다. 민소프트는 프로젝트의 크기가 아닌 고객 만족을 최우선으로 합니다." },
              { q: "자체 솔루션 '베베헬퍼'는 어떤 서비스인가요?", a: "민소프트의 기술력을 바탕으로 육아 환경의 혁신을 목표로 하는 페어런팅 테크 솔루션입니다. 현재 정밀 개발 단계에 있으며 런칭 시 사이트를 통해 안내 드릴 예정입니다." },
              { q: "대형 SI 업체와 비교했을 때의 강점은?", a: "영업이나 관리 비용 거품을 제거했습니다. 대표가 직접 소통하고 실무 정예진이 즉각 대응하기에 소통 비용과 실제 견적이 합리적입니다." }
            ].map((faq, i) => (
